...
Hooks.once('tokenActionHudCoreApiReady', async (coreModule) => {
    ...
    TOR2EActionHandler = class TOR2EActionHandler extends coreModule.api.ActionHandler {
        ...
        async _loadWeapons() {
            if(this.actor.type === 'character') {
                ...
                weapons.filter(w => w?.system?.equipped?.value).forEach(weapon => {
                    ...
                    const proficiency = proficiencies.get(weapon?.system?.group?.value);
                    ...
                    const action = {
                        ...
                        encodedValue: ['weapon', 'character', weapon.name].join(this.delimiter),
                        info1: {class: "hud-info", text: generateDiamonds(proficiency.value)},
                        info2: {class: "hud-info", text: damage},
                        info3: {class: "hud-info", text: injury},
                    };
        ...
        async _loadTraits() {
            ...
            await this._loadFlaws();
            await this._loadVirtues();
            await this._loadRewards();
        }
        async _loadFeatures() {
            if(this.actor.type === 'character') {
                const feature = this.actor.items.filter(e => e.type === 'trait' && e.system.group.value === 'distinctiveFeature');
                feature.forEach( item => {
                    ...
                    this.addActions([{
                        ...
                        img: item.img,
                        tooltip: description,
                        encodedValue: ['traits','character', 'distinctiveFeature', item.id].join(this.delimiter),
                    }], this.GROUP.features)
                })
            }
        }
        async _loadFlaws() {
            if(this.actor.type === 'character') {
                const flaws = this.actor.items.filter(e => e.type === 'trait' && e.system.group.value === 'flaw');
                flaws.forEach( item => {
                    let description = item.system.description.value;
                    this.addActions([{
                        id: item.id,
                        ...
                        img: item.img,
                        tooltip: description,
                        encodedValue: ['traits','character', 'flaw', item.id].join(this.delimiter),
                    }], this.GROUP.flaws)
                })
            }
        }
        async _loadVirtues() {
            if(this.actor.type === 'character') {
                const traits = this.actor.items.filter(e => e.type === 'virtues');
                traits.forEach( item => {
                    let description = item.system.description.value;
                    this.addActions([{
                        id: item.id,
                        ...
                        img: item.img,
                        tooltip: description,
                        encodedValue: ['traits','character',item.type, item.id].join(this.delimiter),
                    }], this.GROUP.virtues)
                })
            }
        }
        async _loadRewards() {
            if(this.actor.type === 'character') {
                const traits = this.actor.items.filter(e => e.type === 'reward');
                traits.forEach( item => {
                    let description = item.system.description.value;
                    this.addActions([{
                        id: item.id,
        ...
        async _loadHealth() {
            if (this.actor.type === 'character') {
                const effects = this.actor.effects.map(e => e?.name);
                Object.keys(this.actor.system.stateOfHealth).forEach(s => {
                    const active = effects.includes(s);
                    const value = this.actor.system.stateOfHealth[s]?.value;
                    const tooltip = coreModule.api.Utils.i18n("tokenActionHud.tor2e.health.status." + s);
                        ...
                        tooltip: tooltip,
                        encodedValue: ['health','character', s].join(this.delimiter),
                        info1: {text: value}
